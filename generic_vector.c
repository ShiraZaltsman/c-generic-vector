...
struct Vector {
    void *m_data;          /* array size of capacity */
    size_t m_capacity;
...
ErrorCode vectorInsert(Vector *vector, void *value, size_t index) {
    ...
    char *src = (char *) vector->m_data + (vector->data_size * index);
    char *dest = src + vector->m_num_items;
    size_t num_bytes = vector->m_num_items * (vector->size - index);
